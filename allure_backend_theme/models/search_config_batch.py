...
class GlobalSearchConfigBatch(models.Model):
    ...
    def write(self, vals):
        ...
        return super(GlobalSearchConfigBatch, self).write(vals)

    @api.model_create_multi
    def create(self, vals_list):
        '''Override check the values'''
        for vals in vals_list:
            if 'template_id' in vals.keys() and vals.get('template_id', False):
                vals['model_id'] = self.env['global.search.config.template'].search([('id', '=', vals['template_id'])]).model_id.id
    ...
    def _onchange_template_id(self):
        ...
        self.field_ids = [(6, 0, self.template_id.field_ids.ids)]
