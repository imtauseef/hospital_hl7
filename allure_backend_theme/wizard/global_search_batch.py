...
class GlobalSearchBatchUsers(models.TransientModel):
    ...
    def default_get(self, fields):
        vals = super(GlobalSearchBatchUsers, self).default_get(fields)
        if self.env.context.get('active_id', False):
            batch_id = self.env['global.search.config.batch'].browse(self.env.context['active_id'])
            vals.update({
                'batch_id': batch_id.id,
                'model': batch_id.model_id.model
            })
        return vals

    user_ids = fields.Many2many('res.users', 'global_search_user_rel', 'global_search_id', 'user_id', string="Users")
    batch_id = fields.Many2one('global.search.config.batch', string='Global Search Batch', copy=False)
