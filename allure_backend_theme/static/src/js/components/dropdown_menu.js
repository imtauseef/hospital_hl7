odoo.define("allure_backend_theme.DropdownMenu", function(require) {
    'use strict';
    ...
    patch(DropdownMenu.prototype, 'allure_backend_theme.DropdownMenu', {
        willStart() {
            ...
            if (!this.props.manualyClose) {
                ...
                if (
                    ...
                    if (this.props.startOpen && this.el.contains(ev.target)) {
                        ...
                        if ($(ev.target).parents('.o_action_allure').length) {
                            this.state.open = false;
                        }
                    }
                } else {
                    if (!this.state.open && !this.props.startOpen && $(document.querySelectorAll('.o_action_allure')).length === 1) {
                        this.state.open = true;
                    }
                }
            }
