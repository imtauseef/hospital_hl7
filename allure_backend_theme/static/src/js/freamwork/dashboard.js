odoo.define('allure_backend_theme.DashboardCustomizeTheme', function(require) {
    ...
    var CustomizeThemeDialog = Dialog.extend({
        ...
        events: {
            ...
            'click .oe_theme_color': '_onClickSelectTheme',
        ...
        _setSelectedTheme: function(result, init) {
            ...
            _.each(result, function(value, field) {
                if (!_.contains(['font_type_values', 'base_menu', 'base_form_tabs', 'tab_configration', 'box_type', 'radio_type', 'mode', 'base_menu_icon'], field)) {
                    self.$('input[name=' + field + ']').minicolors('value', value);
                } else {
                    if (!_.contains(['base_form_tabs', 'font_type_values', 'base_menu', 'box_type', 'radio_type', 'tab_configration', 'mode'], field)) {
                        ...
                        self.$('input[value="' + self.themeData.box_type + '"]').prop("checked", true);
                        self.$('input[value="' + self.themeData.radio_type + '"]').prop("checked", true);
        ...
        _removeTheme: function($li, res_id) {
            ...
            self._rpc({
                ...
                $li.remove();
                self.displayNotification({ title: _t('"Success"'), message: _t("Theme removed successfully.") });
                if (need_to_reload) {
                    self.close(true);
                    location.reload();
                }
        ...
        _updateRecord: function(theme_id, form_values) {
            ...
            if (form_values) {
                user_vals = {
                    ...
                    'box_type': form_values.box_type,
                    'radio_type': form_values.radio_type,
                };
            }
            return this._rpc({
                model: 'ir.web.theme',
        ...
        _notifyInvalidFields: function(invalidFields) {
            ...
            // warnings.unshift('<ul>');
            // warnings.push('</ul>');
            this.displayNotification({ type: 'danger', title: _t("Some fields are invalid"), message: '' });
        },
        _doChangeTheme: function(theme_id) {
            var self = this;
            self.displayNotification({ title: _t('"Success"'), message: _t("Theme customized successfully.") });
            // self.close(true);
            location.reload();
            return;
        },
        _onClickSaveTheme: async function() {
            var self = this,
                theme_id;
            var form_fields = this.$('.o_control_form').serializeArray();
            await _.each(form_fields, function(input) {
                if (input.value !== '') {
        ...
        _ontabClick: function(event) {
            var self = this;
            _.each($('.oe_teb_view'), function (tab) {
                $(tab).find('a').removeClass('active');
                $(tab).removeClass('active');
            })
            var anchor = $(event.target).closest('.oe_teb_view').find('a')
            anchor.addClass('active');
            var contentId = anchor.data('id');
            if (contentId) {
    ...
    var DashboardCustomizeTheme = Widget.extend({
        ...
        willStart: function() {
            ...
            var acl = self.getSession().user_has_group('allure_backend_theme.group_theme_setting_user').then(function(is_theme_access) {
