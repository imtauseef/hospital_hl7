...
patch(SearchBar.prototype, 'web/static/src/search/search_bar/search_bar.js', {
    ...
    _onClickSearchButton(e) {
        var parentNode = $(e.target).parents('.o_view_controller');
        var searchOptions = $(e.target).parents('.o_control_panel').find('.o_search_options');
        parentNode.toggleClass('ad_open_search');
        searchOptions.toggleClass('o_hidden');
    },
});