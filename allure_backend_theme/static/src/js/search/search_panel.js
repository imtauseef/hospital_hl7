odoo.define("allure_backend_theme.SearchPanel", function(require) {

    const { Model, useModel } = require('web.Model');
    const SearchPanel = require('web.searchPanel');
    const { device } = require("web.config");
    // const { hooks } = owl;
    const { useExternalListener, useSubEnv } = owl;
    const { patch } = require('web.utils');

    patch(SearchPanel.prototype, 'allure_backend_theme.SearchPanel', {
