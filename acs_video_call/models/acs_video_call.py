...
class AcsVideoCall(models.Model):
    ...
    def create(self, vals_list):
        for vals in vals_list:
            vals['name'] = self.env['ir.sequence'].next_by_code('acs.video.call') or ''
    ...
    def action_send_invitaion(self):
        ...
        '''
        self.ensure_one()
        template_id = self.env['ir.model.data']._xmlid_to_res_id('acs_video_call.acs_video_call_invitation', raise_if_not_found=False)
        try:
